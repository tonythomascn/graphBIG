...
//      use atomic instruction & warp vote function
//
// Reference: 
        ...
        unsigned color) 
{
    ...
    if((v1+CHUNK_SZ)>inworklist.get_item_num())
    {
        if ( inworklist.get_item_num()>v1 ) 
            chk_sz =  inworklist.get_item_num()-v1;
        else 
            return;
    }
        ...
        uint64_t edge_cnt)
{
    ...
    while(wl_size!=0)
    {
        kernel<<<num_block_chunked, num_thread_per_block>>>(device_vpl, device_rand,
        ...
        num_thread_per_block = (unsigned int) ceil(wl_size/(double)WARP_SZ)*WARP_SZ;
        if (num_thread_per_block > devProp.maxThreadsPerBlock)
            num_thread_per_block = devProp.maxThreadsPerBlock;
        num_block = (unsigned int)ceil( wl_size/(double)num_thread_per_block );
        num_block_chunked = (unsigned int)ceil( num_block*ratio);
